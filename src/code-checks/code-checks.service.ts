...
import {
  ...
  getErrorMessage,
  getInvalidCodes,
  getCodeFieldRepoList,
} from './code-checks-utils';
...
export class CodeChecksService {
  ...
  async runChecks(payload: EmissionsImportDTO): Promise<string[]> {
    this.logger.debug('Running Code Checks');
    ...
    const promises = [];
    CODE_FIELD_REPO_LIST.forEach(fr =>
      promises.push(
        getInvalidCodes(
          codesMap.get(fr.codeField),
          fr.codeRepo,
          this.entityManager,
        ),
      ),
    ...
    try {
      ...
      results.forEach((result, idx) => {
        const errors = result.map(invalidCodeObj =>
          getErrorMessage(
            CODE_FIELD_REPO_LIST[idx].codeField,
            invalidCodeObj.column1,
          ),
        );
  ...
  collectDailyEmissionsCodes(
    payload: DailyEmissionImportDTO[],
  ...
  collectWeeklyTestSummaryCodes(
    payload: WeeklyTestSummaryImportDTO[],
    codesMap: Map<string, Set<string>>,
  ) {
    if (!payload) return;
  ...
  collectSummaryValueCodes(
    payload: SummaryValueImportDTO[],
    codesMap: Map<string, Set<string>>,
  ) {
    if (!payload) return;
  ...
  collectDailyTestSummary(
    payload: DailyTestSummaryImportDTO[],
    codesMap: Map<string, Set<string>>,
  ) {
    if (!payload) return;
  ...
  collectHourlyOperatingData(
    payload: HourlyOperatingImportDTO[],
    codesMap: Map<string, Set<string>>,
  ) {
    if (!payload) return;
  ...
  collectLongTermFuelFlowData(
    payload: LongTermFuelFlowImportDTO[],
    codesMap: Map<string, Set<string>>,
  ) {
    if (!payload) return;
  ...
  collectSorbentTrapData(
    payload: SorbentTrapImportDTO[],
    codesMap: Map<string, Set<string>>,
  ) {
    if (!payload) return;
  ...
  collectNsps4tSummary(
    payload: Nsps4tSummaryImportDTO[],
    codesMap: Map<string, Set<string>>,
  ) {
    if (!payload) return;
  ...
  collectCode(datum, codeField: string, codesMap: Map<string, Set<string>>) {
    if (isUndefinedOrNull(datum[codeField])) return;

    if (!codesMap.has(codeField)) codesMap.set(codeField, new Set<string>());
