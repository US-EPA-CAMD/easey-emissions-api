...
describe('-- Hourly Apportioned Emissions Controller --', () => {
  ...
  beforeAll(async () => {
    const module = await Test.createTestingModule({
      ...
      hourlyApportionedEmissionsController = module.get(HourlyApportionedEmissionsController);
  ...
  describe('* getHourlyEmissions', () => {
    ...
    it('should return test 1', async () => {
      ...
      jest.spyOn(hourlyApportionedEmissionsService, 'getHourlyEmissions').mockResolvedValue(expectedResult);
      expect(await hourlyApportionedEmissionsController.getHourlyEmissions(paramsDto, req)).toBe(expectedResult);
    });
  });
 });